...
class Users extends ResourceController {
	...
	public function getExperties() {
		...
		$row = $this->common_model->getExpertise();
		if($row){
			$output['status'] = 1; 
...
office_hours:jyvvu*/
	 ...
	 //https://www.webwiders.com/WEB01/Motherocity/api/home-screen?user_id=3
		if (isset($_REQUEST['user_id']) && !empty($_REQUEST['user_id'])) {
			...
			if ($run) {
				...
				$homeComing = date('Y-m-d', strtotime($deliveryDate. ' +30 days'));
				...
				//tip of the day
				$tip = $this->common_model->getTipOfTheDay($finaCurrentWeek, date('w'));
  ...
  /*multiple image upload */
	...
	public function getStages(){
		$data = $this->common_model->GetAllData('stage_management','','id','desc');
	...
	public function getSpecialistDetail() {
		...
		$specialistData = [];
		$specialistReviews = [];
		$this->validation->setRule('specialist_id','specialist_id','required');		
		$this->validation->setRule('user_id','user_id','required');		
		if($this->validation->withRequest($this->request)->run()==false) {	   
			$output['errors']=$this->validation->getErrors();
			$output['message']='check parameters';
		...
		try{
			$userId = $this->request->getVar('specialist_id');
			$specialistData =  $this->common_model->getSpecialistDetail($userId);
