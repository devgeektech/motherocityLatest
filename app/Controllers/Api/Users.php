...
class Users extends ResourceController {
	...
	public function get_nutrition_category() {
		...
		$row = $this->common_model->getToolkitCategory('nutrition_category','','id','desc');
			if($row){
				$result = array();
					foreach($row as $key => $value){
						array_push($result, $value);
	...
	public function get_toolkit_category() {
		...
		try{
			$row = $this->common_model->getToolkitCategory();
			// print($row);
			// die;
			if($row){
				$result = [];
				...
				foreach($row as $r){
					...
					$relFormat = [];
					//related content
					if(!empty($r['related_content'])){
						$related_content =  explode(',',$r['related_content']);
						if($related_content){
							foreach($related_content as $rc){
								$arr = $this->common_model->getBlogById($rc);
								array_push($relatedArr,$arr[0]);
							}

							if($relatedArr){
								foreach($relatedArr as $blg){
									$relFormat[] = array(
										"id" => $blg->id,
										"title" => $blg->title,
										"description" => $blg->description,
										"featured_image" => !empty($blg->featured_image) ? base_url($blg->featured_image) : null,
										"summary" => $blg->summary,
										"blog_type" => $blg->blog_type,
										"category_title" => $blg->category_title,
										"created_at" => $blg->created_at,
									);
									// array_push($relFormat,$arr2[0]);
								}
							}							
						}
					}
					...
					$parent = $r['id'];
					$subCategories = $this->common_model->GetAllData('nutrition_category',array('parent'=>$parent));
					//Subcategories
					if($subCategories){
						foreach($subCategories as $subc){
							$subSubRelatedContent = [];
							$subrelatedArr = [];
							$subrelFormat = [];

							if(!empty($subc['related_content'])){
								$related_content =  explode(',',$subc['related_content']);
								if($related_content){
									foreach($related_content as $rc){
										$arr = $this->common_model->getBlogById($rc);
										array_push($subrelatedArr,$arr[0]);
									}
		
									if($subrelatedArr){
										foreach($subrelatedArr as $blg){
											$subrelFormat[] = array(
												"id" => $blg->id,
												"title" => $blg->title,
												"description" => $blg->description,
							...
							$subCat[] = array(						
								...
								"content_type"=> $subc['content_type'],
								"preview"=> $subc['preview'],
								"related_content" => $subrelFormat,
								"image"=> !empty($subc['image']) ? base_url($r['image']) : '',
					...
					$result[] = array(
						...
						"image"=> !empty($r['image']) ? base_url($r['image']) : '',
						"related_content"=> $relFormat,
	...
	public function oldsignup() {
		...
		$insertData['password']=md5($this->request->getVar('password'));
	...
	*/
	...
	public function signup_otp() { 
		...
		if(empty($this->request->getVar('login_type'))){
			$this->validation->setRule('password','Password','trim|required');
		}
		...
		if(!empty($this->request->getVar('login_type')) && $this->request->getVar('login_type') == "social"){
			$insertData['login_type'] = 'social';
			$insertData['social_provider'] = $this->request->getVar('provider');
		}
		...
		$phone = $phone;
		// if($this->request->getVar('user_type') == 1){
		// 	$phone = "+917901893457";
		// }
		$send = $this->Send_sms->send($phone,$msg);
...
office_hours:jyvvu*/
	...
	public function resetPassword() {
		...
		$confirm_password = md5($this->request->getVar('confirm_password'));
	...
	public function get_user_by_id() {
		...
		if($data) {
			$userData = $this->UserData($data->id);
			$userData['trial_expired'] = false;

			if($userData){
				$planId = $userData['plan_id'];
				$createdAt = date('Y-m-d', strtotime($userData['created_at']. ' + 7 days'));
				if($planId == 0 && $createdAt < date('Y-m-d')){
					$userData['trial_expired'] = true;
				}
			}
	 ...
	 //https://www.webwiders.com/WEB01/Motherocity/api/home-screen?user_id=3
		if (isset($_REQUEST['user_id']) && !empty($_REQUEST['user_id'])) {
			...
			if ($run) {
				...
				$date2_ts = strtotime($deliveryDate);
				...
				$diff = strtotime($current_date) - strtotime($deliveryDate);
  
				// 1 day = 24 hours
				// 24 * 60 * 60 = 86400 seconds
				$finaCurrentWeek =  intval( (abs(round($diff / 86400)) ) / 7 );
			    // echo $finaCurrentWeek;
				// die;
				$weekPercentage = intval( ($finaCurrentWeek / 52 ) * 100 ).'%';

				$month = date('d-m-Y', strtotime($deliveryDate. ' +30 days'));
				
				//calculate Milestones
				$homeComing = date('Y-m-d', strtotime($deliveryDate. ' +1 days'));
				$connection = date('Y-m-d', strtotime($homeComing. ' +3 months'));
				$acceptance = date('Y-m-d', strtotime($connection. ' +3 months'));
				$discovery = date('Y-m-d', strtotime($acceptance. ' +3 months'));
				$inspiration = date('Y-m-d', strtotime($discovery. ' +3 months'));

				if( $homeComing <= $current_date){
					$val["milestone_phase"] = 2;
				}
				elseif($connection <= $current_date){
					$val["milestone_phase"] = 3;
				}
				elseif($acceptance <= $current_date){
					$val["milestone_phase"] = 4;
				}
				elseif($discovery <= $current_date){
					$val["milestone_phase"] = 5;
				}
				elseif($inspiration <= $current_date){
					$val["milestone_phase"] = 6;
				}else{
					$val["milestone_phase"] = 1;
				}
				
				if( $homeComing == $current_date){
					$val["completed_milestone_today"] = 1;
				}
				elseif($connection == $current_date){
					$val["completed_milestone_today"] = 2;
				}
				elseif($acceptance == $current_date){
					$val["completed_milestone_today"] = 3;
				}
				elseif($discovery == $current_date){
					$val["completed_milestone_today"] = 4;
				}
				elseif($inspiration == $current_date){
					$val["completed_milestone_today"] = 5;
				}
				
				$val["current_week"] = $finaCurrentWeek;
				$val["week_percentage"] = $weekPercentage;
				...
				//tip of the day
				$tip = $this->common_model->getTipOfTheDay($weekCount, $dayReminder);
				$blogs = $this->common_model->homePickedBlogs();
                 $reminders = $this->common_model->getReminders($user_id);
				$primaryBlogs = $this->common_model->getPrimaryBlogs();
				$secondryBlogs = $this->common_model->getSecondryBlogs();
				if ($tip) {
					$tipsData = [];
					$relatedArr = [];
					$relatedFormatted = [];
					if(!empty($tip['related_content'])){
						$related_content =  explode(',',$tip['related_content']);
						if($related_content){
							foreach($related_content as $rc){
								$arr = $this->common_model->getBlogById($rc);
								array_push($relatedArr,$arr[0]);
					...
					$tipsData = array(
						...
						"days"=> $tip['days'],
						"preview"=> $tip['preview'],
						"related_content"=> $relatedFormatted,
						"created_at"=> $tip['created_at'],
						"tips_date"=> $tip['tips_date'],
					);
				 	$val["tip_of_the_day"] = $tipsData;
                ...
                if ($reminders) { 
					...
					foreach($reminders as $reminders){ 
						$remindersArr[] = array(
							...
							"meridiem" => $reminders->meridiem,
							"date" => $reminders->date,
							"repeat_type" => $reminders->repeat_type,
							"notes" => $reminders->notes,
						);
					}
					$val["reminders"] = $remindersArr;
				} else {
					$val["reminders"] = array();					 	
				}
				...
				if ($secondryBlogs) { 
					$secondryBlogsArr = [];
					foreach($secondryBlogs as $secondryBlogs){ 
						$secondryBlogsArr[] = array(
							"id" => $secondryBlogs->id,
							"title" => $secondryBlogs->title, 
							"featured_image" => !empty($secondryBlogs->featured_image) ? base_url($secondryBlogs->featured_image): null, 
							"blog_type" => $secondryBlogs->blog_type,
							"category_title" => $secondryBlogs->category_title,
							"created_at" => $secondryBlogs->created_at, 
						);
					}
					$val["secondryBlogs"] = $secondryBlogsArr;
				} else {
					$val["secondryBlogs"] = array();					 	
				}
				if ($primaryBlogs) { 
					$primaryBlogsArr = [];
					foreach($primaryBlogs as $primaryBlogs){ 
						$primaryBlogsArr[] = array(
							"id" => $primaryBlogs->id,
							"title" => $primaryBlogs->title, 
							"featured_image" => !empty($primaryBlogs->featured_image) ? base_url($primaryBlogs->featured_image): null, 
							"blog_type" => $primaryBlogs->blog_type,
							"category_title" => $primaryBlogs->category_title,
							"created_at" => $primaryBlogs->created_at, 
						);
					}
					$val["primaryBlogs"] = $primaryBlogsArr;
				} else {
					$val["primaryBlogs"] = array();					 	
				}
				//Plan Data
				$planId = $run['plan_id'];

				// if($planId && $planId != 0){
				// 	$planData = $this->common_model->GetSingleData(
				// 		"transaction",
				// 	 	["user_id"=>$run['id'], "plan_id"=>$planId],
				// 	  	"id",
				// 	   	"desc"
				// 	);
				...
				// 	if($planData){
				// 		echo $endDate = $planData['end_date'];
				// 		exit;
				// 	}
				// }
	...
	// }
		$tipsData = [];
		$tip = $this->common_model->GetAllData("tips_management", array(), "id", "desc");
			 foreach($tip as $key => $getData){
				$relatedFormatted = $this->common_model->getBlogById($getData['related_content']);
					 $tipsData[] = array(
						"id"=> $getData['id'],
						"title"=> $getData['title'],
						"description"=> $getData['description'],
						"is_free"=> $getData['is_free'],
						"price"=> $getData['price'],
						"blog_id"=> $getData['blog_id'],
						"week_no"=> $getData['week_no'],
						"days"=> $getData['days'],
						"preview"=> $getData['preview'],
						"related_content"=> $relatedFormatted,
						"created_at"=> $getData['created_at'],
						"tips_date"=> $getData['tips_date'],
					);
			 } 
		if ($tipsData) {

			 $output['data']= $tipsData;
			 $output['message']='Records found';
			 $output['status']= 1;
		} else {
			$output['message']='Records found';
	...
	private function UserData($id) {
    	...
    	if($run) {
			...
			$user['profile_type'] = $run['profile_type'];
    ...
    public function aboutUs() { 
    	...
    	$run = $this->common_model->GetSingleData("content_management", array("id"=>7));
    	$response['data'] = $run;
    	$response['status'] = 1;
		$response['message'] = "success"; 		 
		return $this->respond($response);
	...
	public function change_password() {
		...
		$user_pass = md5($this->request->getVar('current_password'));
		$New_Password['password'] = md5($this->request->getVar('new_password'));
	...
	public function change_password_without_current_password() {
  ...
  /*multiple image upload */
	...
	public function socialLogin() {
		$this->validation->setRule('firebase_token', 'firebase_token', 'required');
		$this->validation->setRule('user_type','user_type','required');
		if($this->validation->withRequest($this->request)->run()==false) {
			$output['errors']=$this->validation->getErrors();
			$output['message']='check parameters';
			$output['status']= 2;     
		...
		$firebaseToken = $this->request->getVar('firebase_token');
		//init firebase
		$firebase = (new Firebase\Factory())->withServiceAccount('./firebase.json')->withDatabaseUri('motherocity.firebaseapp.com');
		...
		$userMetaData = $user->providerData[0];
	
		if($userMetaData){
			$userUid = $userMetaData->uid;
			$userEmail = $userMetaData->email;
			$userName = $userMetaData->displayName;
			$userProviderId = $userMetaData->providerId;
			$run = $this->common_model->GetSingleData('users', "phone = '' or email = '".$userEmail."'");
			...
			if($run) {
				...
				$returnArr = array(
					...
					"name"=> $rn['name'],
					"email"=> $rn['email'],
					"profile_image" => $rn['profile_image'],
					"country"=> $rn['country'],
					"phone"=> $rn['phone'],
					"phone_withcode"=> $rn['phone_withcode'],
					"is_phone_verified"=> $rn['is_phone_verified'],
					"user_type"=>  $rn['user_type'],
					"is_verified"=>  $rn['is_verified'],
					"status"=> $rn['status'],
					"birth_type_id"=> $rn['birth_type_id'],
					"plan_id"=> $rn['plan_id'],
					"why_should_call"=> $rn['why_should_call'],
					"residency"=> $rn['residency'],
					"delivery_date"=> $rn['delivery_date'],
					"was_your_birth"=> $rn['was_your_birth'],
					"via_baby_born"=> $rn['via_baby_born'],
					"lang"=> $rn['lang'],
					"lat"=> $rn['lat'],
					"lng"=> $rn['lng'],
					"profile_type"=> $rn['profile_type'],
					"profile_color"=> $rn['profile_color'],
					"is_profile_complete"=> $rn['is_profile_complete'],
					"is_deactive"=> $rn['is_deactive'],
					"is_delete"=> $rn['is_delete'],
					"current_week"=> $rn['current_week'],
					"week_start_date"=> $rn['week_start_date'],
				...
				$output['status'] = 1;
				$output['already_register'] = true;
				$output['message'] = 'User already registered';
			} else {
				$output['data'] = ['name'=>$userName,'email'=>$userEmail,'provider'=>$userProviderId];
				$output['status'] = 1;
				$output['already_register'] = false;
				$output['message'] = 'New user, please verify OTP';
			}
		}
		else{
			$output['data'] = [];
			$output['status'] = 0;
			$output['message'] = 'There is an error on social login';
		}
		return $this->respond($output);
		exit();
	}
	...
	public function getMilestones(){
		if(isset($_REQUEST['user_id']) && !empty($_REQUEST['user_id'])) {
			...
			if ($run) {
				...
				$deliveryDate = $run['delivery_date'];
				// Calculating the difference in timestamps
				$diff = strtotime($deliveryDate) - strtotime($current_date);
				...
				$output['status']= 1;
				$output['message']= 'Milestones data fetched';
				
			}
			else{
				$output['message']='No user found with the id';
				$output['status']= 0;
			}
		}
		else {
	...
	public function getStages(){
		$data = $this->common_model->GetAllData('stage_management','','id','asc');
		
		$output['data'] = $data;
		$output['status']= 1;
		$output['message']= 'Stages data fetched';
	...
	public function specialistsList(){
		...
		try{
			if(isset($_REQUEST['user_id']) && $_REQUEST['user_id'] != ""){
				...
				//lat long filter
				if(isset($_REQUEST['lat_long']) && $_REQUEST['lat_long'] != ""){
					$lat_long = $_REQUEST['lat_long'];
					$explode = explode(',', $lat_long);
					if(sizeof($explode) > 1){
						$userCurrentLocation->lat = $explode[0];
						$userCurrentLocation->long = $explode[1];
					}
				}

				if($userCurrentLocation->lat && $userCurrentLocation->lng){
					$nearbySpecialists = $this->common_model->getDistance($userCurrentLocation->lat, $userCurrentLocation->lng, $categoryId);

					if($nearbySpecialists){
						foreach($nearbySpecialists as $ns){
							$splArr = [];
							//Get review average and totla count
							$reviewData = $this->common_model->getAverageReviews($ns['id']);
							if($reviewData){
							...
							//get expertise
							// $speciality = $this->common_model->getSpecialityById($ns['your_expertise']);
							// if($speciality){
							// 	$mySpeciality = $speciality->title;
							// }

							$specialistReviews = $this->common_model->getSpecialistReview($ns['id']);
							if($specialistReviews){
								$splArr = $specialistReviews;
							}
							$nearbySpecialistsArr[] = array(
								'id' => $ns['id'],
								'name' => $ns['name'],
								'email' => $ns['email'],
								'profile_image' => base_url().'/'.$ns['profile_image'],
								'country' => $ns['country'],
								'phone' => $ns['phone'],
								'user_type' => $ns['user_type'],
								'office_area' => $ns['office_area'],
								'distance' => round($ns['distance'],2) .' Miles',
								'total_reviews' => $totalReviews,
								'avg_rating' => $avgRating,
								 'speciality' => $mySpeciality,
								 'few_word' => $ns['few_word'],
								 'reviews' => $splArr
							);
						}

						$output['data'] = $nearbySpecialistsArr;
						$output['status'] = 1;
						$output['message'] = 'Specialists Data';
						return $this->respond($output);
					}	
					else{
						$output['data'] = [];
						$output['status'] = 0;
						$output['message'] = 'No nearby specialists found';
						return $this->respond($output);
					}
				}
				else{
					$output['data'] = [];
					$output['status'] = 0;
					$output['message'] = 'No location found, please update your current location';
					return $this->respond($output);
				}
			}
			else{
				$output['data'] = [];
				$output['status'] = 0;
				$output['message'] = 'user_id cannot be null';
				return $this->respond($output);
			}
		}
		catch (\Exception $e) {
    ...
    //Add review for specialists
	public function addSpecialistReview() {
	 	$attachments = [];
	 	$makeArr = [];
		...
		try{
			$specialistId = $this->request->getVar('specialist_id');
			$userId = $this->request->getVar('user_id');
			// check if already exists
			$checkExists = $this->common_model->GetSingleData(
				'specialist_reviews',
				['user_id' => $userId, 'specialist_id' => $specialistId]
			);

			if($checkExists){
				$output['message'] = 'You already added a review for the specialist';
				$output['status'] = 0 ;
				return $this->respond($output);
			}
			...
			$insertData['rating'] = $this->request->getVar('rating');		
	       ...
	       //      if(move_uploaded_file($tmp_name,$path)){
	       //        $attachments[] = base_url().'/'.$path;
	       //      }
	       //    }
	       //  } 
	        
	        $attachments = $this->request->getVar('image');
	        if( !empty($attachments) ){
	        	$makeArr = explode(',', $attachments);

	        	if(sizeof($makeArr) > 0){
	        		$attachments = json_encode($makeArr);
	        	}


	        }
	        
	        $insertData['files'] = $attachments;
	        	 
			$run = $this->common_model->insertData('specialist_reviews', $insertData);

			if($run) {
				$output['data'] = $this->common_model->GetDataById('specialist_reviews',$run);;
	...
	public function getSpecialistDetail() {
		$blogIds = [];
		$getBlogDetails = [];
		$specialistData = [];
		$specialistReviews = [];
		$this->validation->setRule('specialist_id','specialistID','required');		
		$this->validation->setRule('user_id','userID','required');		
		if($this->validation->withRequest($this->request)->run()==false) {	   
			$output['errors']=$this->validation->getErrors();
			$output['message']='check parameters';
			$output['status']= 2;     
		...
		try{
			$specialistID = $this->request->getVar('specialist_id');
			$userID = $this->request->getVar('user_id');
			 $specialistData =  $this->common_model->getSpecialistDetail($specialistID);
			 $specialistExist =  $this->common_model->getSpecialistSave($specialistID,$userID);  
		  if(isset($specialistExist->specialist_id))
		  {
			  $already_saved= 'true';
		  }
		  else{
			...
			if(isset($specialistData[0]->id) && $specialistData[0]->id != null) {
				$specialistReviews = $this->common_model->getSpecialistReview($userId);
				$splArr = $specialistData[0];
				$splArr->reviews = $specialistReviews;

				$output['data'] = $splArr;
				$output['message'] = 'Specialist detail';
				$output['status'] = 1 ;
				$output['already_saved'] = $already_saved;

			} else {
				$output['message'] = 'No specialist found';
				$output['status'] = 0 ;
			}
	...
	public function getAllBlogs(){
		...
		try{
			$normal1 = $this->common_model->getBlogsData(0, 2, $catId);
			$normalExclude = [];
			if($normal1){
				foreach($normal1 as $nm){
					$normalExclude[] = $nm->id;
				}
			}

			$primary = $this->common_model->getBlogsData(2, 1, $catId);

			$primaryExclude = [];
			if($primary){
				foreach($primary as $nm){
					$primaryExclude[] = $nm->id;
				}
			}

			$normal2 = $this->common_model->getBlogsData(0, 3, $catId, $normalExclude);

			$normal2Exclude = [];
			if($normal2){
				foreach($normal2 as $nm){
					$normal2Exclude[] = $nm->id;
				}
			}

			$secondary = $this->common_model->getBlogsData(2, 2, $catId, $primaryExclude);
			
			if($primaryExclude && $normalExclude && $normal2Exclude){
				$finalExclude = array_merge($primaryExclude,$normalExclude,$normal2Exclude);
			}
			...
			$blogArr = array(
				...
				'normal_2' => $normal2,
				'secondary' => $secondary,
				'normal_3' => $this->common_model->getBlogsData(0, 20, $catId, $finalExclude)
			);
    ...
    public function saveDeviceToken(){ 
        ...
        $this->validation->setRule('device_token','Device Token','trim|required');
        ...
        try{   
			$user_id['user_id'] = $this->request->getVar('user_id');		
             $device_token['device_token'] = $this->request->getVar('device_token');	
            $saveDeviceToken = $this->common_model->UpdateData('users', array("id"=>$user_id),$device_token);
			
        if($saveDeviceToken) {
            $output['data'] = $saveDeviceToken;
